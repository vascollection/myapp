...
<script>
  ...
  const editTask = () => {
    if (newTask.trim() !== "") {
      taskToEdit = {id: taskToEdit.id, task: newTask, status: taskToEdit.status }
  ...
  const CheckStatus = (item) => {
    ...
    return targetID(item).status
    // mo return ni sya ug 'active' | 'completed'
  }
...
<div class="container">
  ...
  {#if editCondition}
    <div class="input">
      ...
      <div class="action">
        <button class="buttonStyle" on:click={editTask} type="button"
          >Save</button
        >
  ...
  {:else}
    <div class="input">
      ...
      />
      <button class="buttonStyle" on:click={addToList} type="button"
        >Add task</button
      >
    </div>
  {/if}
